...
$message = '';
if (isset($_COOKIE["showSetupFinish"])) {
	$message = $_COOKIE["showSetupFinish"];
    unset($_COOKIE['showSetupFinish']);
    $res = setcookie('showSetupFinish', '', time() - 3600);
}
...
echo $twig->render('setup.html', array('items_menu' => $items_menu, 'news' => getNews(), 'lang' => getLangV2(), 'setting' => getSettingsV2(), 'user_data' => $user, 'exist_character' => $exist_character, 'character' => $character, "error_login" => $error_login, "server_status" => get_server_status(), "players_online" => get_online_users(), "registered_accounts" => get_registered_accounts(), "index_ranking" => get_index_ranking(), "message" => $message));
remove_session_msg();
?>